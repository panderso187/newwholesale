...
intro_md = """# Lead Harvesting Notebook LM
AI-powered pipeline for scraping public records (probate, evictions, code violations, tax delinquencies), enriching with LLMs, and routing to Google Sheets + Podio CRM, with optional REIRail skip-tracing.

> **Author:** Generated by ChatGPT on {date}\n""".format(date=datetime.now(timezone.utc).strftime("%Y-%m-%d %H:%M UTC"))
cells.append(nbf.v4.new_markdown_cell(intro_md))

# Environment variables cell
env_code = """\
# ╔══════════════════════════════════════════════╗
...
# Scraper template cell
scraper_code = """\
# ╔══════════════════════════════════════════════╗
# ║           SCRAPER TEMPLATE FUNCTION          ║
# ╚══════════════════════════════════════════════╝
def scrape_code_violations(url: str) -> pd.DataFrame:
    \"\"\"Scrape code-violation table from city portal -> DataFrame\"\"\"
    headers = {
...
def score_motivation_llm(row: dict, model: str = "gpt-4o-mini") -> int:
    \"\"\"Return 1-5 motivation score using OpenAI / DeepSeek\"\"\"
    prompt = f"Rate seller motivation 1-5 (5 = desperate, 1 = unlikely) given:\nAddress: {row['address']}\nViolation: {row['violation']}\nStatus: {row['status']}\nReported: {row['date_reported']}\nReturn only the integer."
    try:
...
def run_pipeline(source_url: str):
    ...
    for _, row in df.iterrows():
        ...
        if skip: print("Skip-trace result:", skip)
"""
...
# Save notebook
out_path = Path("/mnt/data/LeadHarvestingNotebook.ipynb")
